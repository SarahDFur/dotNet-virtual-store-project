...
internal static class DataSource
{
    ...
    //Data Lists
    internal static List<Product?> DSProducts { get; } = new List<Product?> { };
    internal static List<Order?> DSOrders { get; } = new List<Order?> { };
    internal static List<OrderItem?> DSOrderItems { get; } = new List<OrderItem?> { };
    ...
    private static void CreateOrders()
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            ...
            while(_order.ShipDate > DateTime.Now) 
            {
                _order.ShipDate = _order.OrderDate + new TimeSpan(s_rand.NextInt64(10L * 1000L * 1000L * 3600L * 24L * 10L));
            }
            ...
            while (_order.DeliveryDate > DateTime.Now)
            {
                _order.DeliveryDate = _order.ShipDate + new TimeSpan(s_rand.NextInt64(10L * 1000L * 1000L * 3600L * 24L * 10L));
            }
            DSOrders.Add(_order);
        }
    }
    ...
    private static void CreateOrderItems() 
    {
        for(int i = 0; i < 20; i++)
        {
            int _orderId = Config.s_startOrderNumber + 1 + i;
            int numOfItems = s_rand.Next(1, 5);
            for (int j = 0; j < numOfItems; j++)
            {
                Product? product = DSProducts[s_rand.Next(DSProducts.Count)]; //choose random product to put into the orderitems list
